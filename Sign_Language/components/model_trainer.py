...
from Sign_Language.entity.artifacts_entity import ModelTrainerArtifact
...
class ModelTrainer:
    ...
    def initiate_model_trainer(self,)-> ModelTrainerArtifact:
        logging.info("Entered initiate_model-trainer method of modelTrainer class")
        try:
            ...
            config=read_yaml_file(f"yolo5/models/{model_config_file_name}.yaml")
            config['nc'] = int(num_classes)

            with open(f'yolo5/models/custom_{model_config_file_name}.yaml', 'w') as f:
                yaml.dump(config,f)
